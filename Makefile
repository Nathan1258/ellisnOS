...
ASM_FLAGS = -f elf32
# compiler flags
CC_FLAGS = $(INCLUDE) $(DEFINES) -m32 -std=gnu99 -ffreestanding -Wall -Wextra --target=i686-elf

# linker flags, for linker add linker.ld file too
LD_FLAGS = @$(CONFIG)/linker.ld -nostdlib

# target file to create in linking
TARGET=$(OUT)/ellisnkernel.bin

# iso file target to create
TARGET_ISO=$(OUT)/ellisnkernel.iso
ISO_DIR=$(OUT)/isodir
